...
export const CITY_BLURBS: Record<string, string> = {
  ...
  "new-york": "New York has more studios competing for the same searches than almost any city we cover — Midtown, the East Village, Brooklyn, Queens — which means a great studio with a thin online presence gets buried fast. A proper listing is how you stay findable in that noise.",
  "los-angeles": "Los Angeles spreads wellness demand across Hollywood, Santa Monica, Downtown and the Valley, so someone searching from one neighbourhood often has no idea what's a few miles over. A city-wide directory closes that gap instead of leaving it to whoever has the biggest ad budget.",
  chicago: "Chicago's Thai massage demand clusters around the Loop, Lincoln Park and River North, and picks up hard in the colder months when indoor wellness searches spike. A studio that's easy to find in that window gets the booking.",
