...
export const cmsSchemas = {
  ...
  article: z.object({ id: z.number().int().positive().optional(), title: z.string().trim().min(6).max(255), slug: z.string().trim().min(6).max(280).regex(/^[a-z0-9]+(-[a-z0-9]+)*$/, "Lowercase letters, numbers and hyphens only — no quotes or spaces."), excerpt: optionalString, body: optionalString, topic: z.string().trim().min(3).max(120), coverImageUrl: optionalString, status: z.enum(["draft", "review", "published"]) }),
