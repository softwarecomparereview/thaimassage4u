...
type Settings = { enabled: boolean; autoPublish: boolean; batchSize: number; concurrency: number; dailyCap: number; model: string; target: "thin" | "missing" | "all" };
...
const TARGETS: Array<{ value: Settings["target"]; label: string; hint: string }> = [
  ...
  { value: "all", label: "Every listing", hint: "Rewrites descriptions that are already fine. Expensive." },
];
...
export default function CmsEnrichment() {
  ...
  const approveAll = useMutation({
    ...
    onSuccess: (result: any) => { toast.success(`${result.approved} description${result.approved === 1 ? "" : "s"} published.`); refresh(); },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  const { settings, models, usedToday, backlog, totals, runs, items } = status.data;
  ...
  return <div className="cms-grid">
    <section className="cms-card">
      <header className="cms-card__head">
        <div>
          ...
          <p>Reads each studio's own website and writes a description from what the page actually says. Runs every 15 minutes on the Worker; nothing needs to stay open.</p>
        </div>
    ...
    <section className="cms-card">
      ...
      <div className="cms-field-grid">
        ...
        <label className="cms-field-grid__toggle">
          ...
          <small>Off by default. While off, every description waits below.</small>
        </label>
      </div>
    </section>
    ...
    <section className="cms-card">
      <header className="cms-card__head">
        <div><p className="eyebrow">Review queue</p><h2>{proposals.length ? `${totals.proposed ?? 0} description${(totals.proposed ?? 0) === 1 ? "" : "s"} waiting.` : "Nothing waiting."}</h2></div>
      ...
      {proposals.length === 0 && <p className="subtle-copy">Start the worker, or run one batch, and proposals land here.</p>}
      {proposals.map(item => <article key={item.id} className="cms-proposal">
        <div>
